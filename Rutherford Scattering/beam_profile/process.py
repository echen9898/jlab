...
# def triangular(x, h, w): # triangle wave fit function
#     func = where(abs(x) <= w, 1-abs(x)/w, 0)
#     return func
...
# mode = 'g' # t for triangular, g for gaussian

# x = array([-6, -4, -2, 0, 2, 4, 6, 8, 10]) * 0.0174533 # x values discrete (radians)
# x_continuous = arange(-10, 12, 0.1) * 0.0174533 # x values continuous (radians)

# if mode == 't':
#     p0 = array([170, 3]) # h, w
#     fit = curve_fit(triangular, x, count_rates, p0, rate_errs, maxfev=500000) # find the best fit with curve_fit
#     h, w = fit[0]
#     print(h, w)

#     plt.plot(x_continuous, triangular(x_continuous, h, w), color='red') # plot the fit
...
#     plt.plot(x_continuous, gaussian(x_continuous, a, mu, std), color='red') # plot the fit

#     #----- fit two lines to gaussian -----#
    
#     x_l = arange(-6*0.0174533, mu, 0.0001) # left side x value range
#     x_r = arange(mu, 10*0.0174533, 0.0001) # right side x value range
...
#     plt.plot(x_l, linear(x_l, m_l, b_l), color='blue')
#     plt.plot(x_r, linear(x_r, m_r, b_r), color='blue')
